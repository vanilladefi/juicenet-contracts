...
import { JuiceStaking, JuiceStaking__factory, MockJuiceStaking__factory } from "../typechain/juicenet"
...
export default async ({ logic: logicAddress }: Arguments, hre: HardhatRuntimeEnvironment): Promise<void> => {
  let { ethers, deployments, network, getNamedAccounts, upgrades } = hre

  if (network.name !== "rinkeby") throw new Error("Only supported on Rinkeby")
  ...
  if (!logicAddress) {
    ...
    stakingLogic.deployTransaction
