...
export default async ({ logic: logicAddress }: Arguments, hre: HardhatRuntimeEnvironment): Promise<void> => {
  ...
  if (!logicAddress) {
    ...
    let juiceStakingFactory = new JuiceStaking01__factory(signer)
    ...
    let stakingLogic = JuiceStaking01__factory.connect(logicAddress, ethers.provider)
    console.log(`Deploying the proxy for logic contract '${logicAddress}' (bytecode equals: ${JuiceStaking01__factory.bytecode === await ethers.provider.getCode(logicAddress)})`)
